		...
		for {
			...
			case <-sc.sendTimer.C:
				...
				if err := p.Send(c.srl); err != nil {
					logutil.BgLogger().Error(fmt.Sprintf("Unable to send '%s' metadata: %v", name, err))
				}
			}
		}
	}()
...
func (c *Scheduler) TriggerAndResetCollectorTimer(name string, delay time.Duration) {
	...
	if !found {
		logutil.BgLogger().Error("Unable to find '" + name + "' in the running metadata collectors!")
	}
...
func (c *Scheduler) firstRun() error {
	...
	if !found {
		logutil.BgLogger().Error("Unable to find 'host' metadata collector in the catalog!")
